...
class VideoInstanceCutter(nn.Module):
    ...
    def readout(self, read_type: str = "last"):
        ...
        if read_type == "last":
            ...
            out_pos_embeds = []
            for seq_id in self.last_seq_ids:
                out_pos_embeds.append(self.video_ins_hub[seq_id].self.similarity_guided_pos_embed)
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher,
        ...
        for i in range(T):
            ...
            track_embeds = self.get_mask_pos_embed(outputs_mask[-1, :, valid_track_query, :, :],
                                                   ori_mask_features[:, i, ...])  # q', b, c
            valid_track_embeds_id = torch.full(size=(outputs_mask.shape[2], ), fill_value=-10000, dtype=torch.int64).to("cuda")
            valid_track_embeds_id[valid_track_query] = torch.arange(0, track_embeds.shape[0]).to(valid_track_embeds_id)
            cur_seq_ids = []
            for k, valid in enumerate(valid_track_query):
                ...
                if valid:
                    if not seq_id in self.video_ins_hub:
                        self.video_ins_hub[seq_id] = VideoInstanceSequence(tgt_ids_for_each_query[k], seq_id)
                    self.video_ins_hub[seq_id].update(track_embeds[valid_track_embeds_id[k], 0, :])
