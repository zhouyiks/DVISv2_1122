...
class VideoInstanceCutter(nn.Module):
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher,
        ...
        for i in range(T):
            ...
            transformed_fq = self.transform_fq(torch.cat([single_frame_embeds_no_norm, disappear_embed], dim=0))
            fq_outputs_class, fq_outputs_mask = self.prediction(transformed_fq[None], mask_features[:, i, ...])
            fq_out_dict = {
                "pred_logits": fq_outputs_class[-1],  # b, q+1, k+1
                "pred_masks": fq_outputs_mask[-1],  # b, q+1, h, w
            ...
            fq_reid_embeds = self.reid_embed(transformed_fq)  # q+1, b, c
            if i > 0:
                tq_reid_embeds = self.reid_embed(self.track_queries)  # q', b, c
                ...
                for tgt_id in tq_tgt2src_dict.keys():
                    if not tgt_id in fq_tgt2src_dict.keys():
                        continue  # TODO, disappear, positive to disappear_embed
                    anchor_embedding = tq_reid_embeds[tq_tgt2src_dict[tgt_id], 0, :]
                    positive_embedding = fq_reid_embeds[fq_tgt2src_dict[tgt_id], 0, :]
                    negative_query_id_1 = sorted(
                        random.sample(set(range(fq_reid_embeds.shape[0])) - set([fq_tgt2src_dict[tgt_id]]),
                                      fq_reid_embeds.shape[0] - 1))
                    reid_out = {
                        "anchor_embedding": anchor_embedding,
                        "positive_embedding": positive_embedding,
                        "negative_embedding": fq_reid_embeds[negative_query_id_1, 0, :]
                    }
                    reid_out_list.append(reid_out)

            # fq_out_dict.update({
            #     "fq_reid_embeds": fq_reid_embeds[:-1],
            #     "tg_fq_reid_embeds": single_frame_reid_embeds,
            # })
