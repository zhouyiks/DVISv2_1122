...
class VideoInstanceCutter(nn.Module):
    ...
    def __init__(
        ...
        self.new_ins_embeds = nn.Embedding(2, hidden_dim)
    ...
    def forward(self, frame_embeds_no_norm, mask_features, targets, frames_info, matcher,
        ...
        new_ins_embeds = self.new_ins_embeds.weight.unsqueeze(1).unsqueeze(0).repeat(50, 1, B, 1).flatten(0, 1)
        for i in range(T):
            ms_output = []
            ...
            if i == 0 and resume is False:
                ...
                fQ_pos = self.get_mask_pos_embed(frames_info["pred_masks"][i][0][None],
                                                   ori_mask_features[:, i, ...])

                fq_pred_logits = frames_info["pred_logits"][i][0]  # q, k+1
                fq_max_scores = torch.max(fq_pred_logits.softmax(-1)[:, :-1], dim=-1)[0]  # q
                _, sorted_indices = torch.sort(fq_max_scores, descending=True)
                detQ_pos = fQ_pos[sorted_indices[:50], :, :].unsqueeze(1).repeat(1, 2, 1, 1).flatten(0, 1)
            ...
            if i == 0 and resume is False:
                ...
                if using_thr:
                    ...
                    #     print(valid_track_query[self.disappear_trcQ_id])
    ...
    def inference(self, frame_embeds_no_norm, mask_features, frames_info, start_frame_id, resume=False, to_store="cpu"):
        ...
        new_ins_embeds = self.new_ins_embeds.weight.unsqueeze(1).unsqueeze(0).repeat(50, 1, B, 1).flatten(0, 1)  # nq, b, c
        for i in range(T):
            ms_output = []
            single_frame_embeds_no_norm = frame_embeds_no_norm[i]  # q, b, c
            valid_fq_mask = frames_info["valid"][i][0]
