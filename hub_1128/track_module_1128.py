...
class VideoInstanceCutter(nn.Module):
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher,
        ...
        for i in range(T):
            ...
            if i > 0:
                tq_reid_embeds = self.reid_embed(
                    torch.cat([self.track_queries, all_invalid_track_queries[-1]], dim=0))
                ...
                for tgt_id in tq_tgt2src_dict.keys():
                    ...
                    reid_out = {
                        ...
                        "negative_embedding": curr_reid_embeds[negative_query_id_1, 0, :]
                    }
            ...
            self.track_embeds = self.get_mask_pos_embed(outputs_mask[-1, :, valid_track_query, :, :],
                                                        ori_mask_features[:, i, ...])  # q', b, c
            all_invalid_track_queries.append(ms_output[-1][~valid_track_query])
