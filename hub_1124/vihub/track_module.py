...
class VideoInstanceCutter(nn.Module):
    ...
    def __init__(
        ...
        self.build_relation_module = nn.Sequential(
            SelfAttentionLayer(
                d_model=hidden_dim,
                nhead=num_head,
                dropout=0.0,
        ...
        for _ in range(self.num_warming_layers):
        ...
        for _ in range(self.num_layers - self.num_warming_layers):
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher,
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                for j in range(self.num_warming_layers):
                    output = self.transformer_cross_attention_layers_denoise[j](
                        indentify=ms_output[-1], tgt=ms_output[-1],
                        key=single_frame_embeds_no_norm,
                        memory=single_frame_embeds_no_norm,
                    )
                    output = self.transformer_self_attention_layers_denoise[j](output)
                    output = self.transformer_ffn_layers_denoise[j](output)
                    ms_output.append(output)
                for j in range(self.num_layers - self.num_warming_layers):
                ...
                new_ins_embed = self.new_ins_embeds.weight.unsqueeze(1)  # 1, b, c
                old_new_embeds = self.build_relation_module(torch.cat([self.track_queries, new_ins_embed], dim=0))
                new_ins_embeds = old_new_embeds[-1:, :, :].repeat(self.num_new_ins, 1, 1)  # nq, b, c
                pilot = torch.cat([self.track_queries, new_ins_embeds], dim=0)

                old_new_reid_embeds = self.reid_embed(old_new_embeds)
                trcQ_reid_embeds, newQ_reid_embeds = old_new_reid_embeds[:-1, ...], old_new_reid_embeds[-1:, ...]
                trcQ_pos_embeds = self.pos_embed(trcQ_reid_embeds)
                fQ_pos_embeds = self.pos_embed(single_frame_reid_embeds)
                pilot_pos = torch.cat([trcQ_pos_embeds, fQ_pos_embeds], dim=0)
                src_pos = torch.cat([fQ_pos_embeds, trcQ_pos_embeds], dim=0)

                disappear_embeds = disappear_embed.repeat(self.track_queries.shape[0], 1, 1)
                src = torch.cat([single_frame_embeds_no_norm, disappear_embeds], dim=0)
                ...
                output = torch.cat([self.track_queries, torch.zeros_like(single_frame_embeds_no_norm)], dim=0)
                ms_output.append(output)
                for j in range(self.num_warming_layers):
                    output = self.transformer_cross_attention_layers_denoise[j](
                        indentify=ms_output[-1], tgt=pilot,
                        key=src, memory=src,
                        query_pos=pilot_pos, pos=src_pos,  #TODO using track_queries instead.
                        memory_mask=attn_mask,
                    )
                    output = self.transformer_self_attention_layers_denoise[j](output)
                    output = self.transformer_ffn_layers_denoise[j](output)
                    ms_output.append(output)
                    pilot = torch.cat([output[:-self.num_new_ins, ...], new_ins_embeds], dim=0)
                for j in range(self.num_layers - self.num_warming_layers):
                    output = self.transformer_cross_attention_layers[j](
                        ms_output[-1], src,
                        memory_mask=attn_mask
            ...
            if i == 0 and resume is False:
                ...
                if stage == 1:
                    indices = []
                    valid_inst = targets_i[0]["valid_inst"]
                    new_inst = torch.zeros_like(valid_inst, dtype=torch.bool)
                    old_inst = self.prev_frame_indices[1]
                    new_inst[valid_inst] = True
                    new_inst[old_inst] = False
                    if torch.all(~new_inst):
                        # no new instances appear
                        indices.append(self.prev_frame_indices)
                    else:
                        tgt2fq_indices = {tgt.item(): fq.item() for tgt, fq in zip(frames_info["indices"][i][0][1],
                                                                                   frames_info["indices"][i][0][0])}
                        new_src_i, new_tgt_i = [], []
                        for k, k_is_new in enumerate(new_inst):
                            if k_is_new:
                                new_src_i.append(self.track_queries.shape[0] + tgt2fq_indices[k])
                                new_tgt_i.append(k)
                        all_src_i = torch.cat([self.prev_frame_indices[0], torch.as_tensor(new_src_i).to(self.prev_frame_indices[0])])
                        all_tgt_i = torch.cat([self.prev_frame_indices[1], torch.as_tensor(new_tgt_i).to(self.prev_frame_indices[1])])
