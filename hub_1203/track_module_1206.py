...
class VideoInstanceCutter(nn.Module):
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher,
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                if self.disappear_tgt_id is not None and continuously_disappear:
                    # should not be discarded by the condition of len(self.tgt_ids_for_track_queries) > 2
                    aux_tgt_i_for_each_fq = frames_info["aux_indices"][i][0][1]
                    disappear_fq_mask[aux_tgt_i_for_each_fq == self.disappear_tgt_id] = True
                    tgt2src_dict = {tgt.item(): src for tgt, src in zip(self.prev_frame_indices[1],
                                                                        self.prev_frame_indices[0])}
                    self.disappear_trcQ_id = tgt2src_dict[self.disappear_tgt_id.item()]
                elif self.tgt_ids_for_track_queries is not None and len(self.tgt_ids_for_track_queries) > 2:
            ...
            if i == 0 and resume is False:
                ...
                if using_thr:
                    ...
                    pred_scores = torch.max(outputs_class[-1, 0].softmax(-1)[:, :-1], dim=-1)[0]
                    valid_track_query = pred_scores > self.inference_select_thr
                    # model keeping inactive object after its disappear
                    if self.disappear_trcQ_id is not None:
                        # if random.random() > 0.3 and torch.sum(valid_track_query) > 2:
                        #     continuously_disappear = True
                        #     valid_track_query[self.disappear_trcQ_id] = True
                        # else:
                        #     continuously_disappear = False

                        if not continuously_disappear and random.random() > 0.7:
                            continuously_disappear = True
                        if continuously_disappear:
                            valid_track_query[self.disappear_trcQ_id] = True

                    # model newly born
                    if torch.sum(valid_track_query) > 2 and continuously_disappear:
                        _tgt_id_for_each_valid_trcQ = tgt_ids_for_each_query[valid_track_query]
                        select_idx = random.randrange(0, _tgt_id_for_each_valid_trcQ.shape[0])
                        select_tgt_id = _tgt_id_for_each_valid_trcQ[select_idx]
                        if select_tgt_id == -1:
                            pass
                        elif len(self.new_born_tgt_ids) > 0:
                            pass
                        elif self.disappear_tgt_id is None or self.disappear_tgt_id != select_tgt_id:
                            tgt2src_dict = {tgt.item(): src for tgt, src in zip(out_dict["indices"][0][1],
                                                                                out_dict["indices"][0][0])}
                            valid_track_query[tgt2src_dict[select_tgt_id.item()]] = False
                            self.new_born_tgt_ids.append(select_tgt_id)
                else:
                    tgt_ids_for_each_query = torch.full(size=(ms_output.shape[1],), dtype=torch.int64,
                                                        fill_value=-1).to("cuda")
                    tgt_ids_for_each_query[indices[0][0]] = indices[0][1]
    ...
    def filter_new_ins(self, pred_scores, pred_masks):
        """
        pred_scores: q'+nq, k+1
        pred_masks: q"+nq, h, w
        """
