...
class VideoInstanceCutter(nn.Module):
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher,
        ...
        assert B == 1
        all_outputs = []
        select_fid = random.randrange(1, T)
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                if i == select_fid:
                    tgt2src = {tgt.item(): src for tgt, src in
                               zip(self.prev_frame_indices[1], self.prev_frame_indices[0])}
                    select_idx = random.randrange(0, len(self.prev_frame_indices[1]))
                    ...
                    if (not using_thr) or len(self.disappeared_tgt_ids) > 0:
                        ...
                        self.disappear_trcQ_id = tgt2src[select_tgt_id.item()]
            ...
            if i == 0 and resume is False:
                ...
                if using_thr:
                    ...
                    if self.disappear_trcQ_id is not None:
                        valid_track_query[self.disappear_trcQ_id] = False  # to be newly born in the next frame
                else:
                    tgt_ids_for_each_query = torch.full(size=(ms_output.shape[1],), dtype=torch.int64,
                                                        fill_value=-1).to("cuda")
                    tgt_ids_for_each_query[indices[0][0]] = indices[0][1]
