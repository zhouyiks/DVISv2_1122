...
class ViHubCriterion(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # warming outputs
        #===================
        warming_outputs = [outputs_i["warming_outputs"] for outputs_i in outputs]
        warming_indices = [warming_outputs_i["indices"] for warming_outputs_i in warming_outputs]
        w_num_masks = 0
        for _target, _output, _indices in zip(targets, warming_outputs, warming_indices):
            valid_mask = _target[0]["valid_inst"][_indices[0][1]]
            if _output["disappear_tgt_id"] > -1:
                w_num_masks += (valid_mask.sum() - 1)
            else:
                w_num_masks += valid_mask.sum()
        w_num_masks = torch.as_tensor(
            [w_num_masks], dtype=torch.float, device=next(iter(outputs[0].values())).device
        )
        if is_dist_avail_and_initialized():
            torch.distributed.all_reduce(w_num_masks)
        w_num_masks = torch.clamp(w_num_masks / get_world_size(), min=1).item()
        # w_num_masks = num_masks

        if "aux_outputs" in warming_outputs[0]:
            len_aux = len(warming_outputs[0]["aux_outputs"])
            for i in range(len_aux):
                aux_outputs = [outputs_i["aux_outputs"][i] for outputs_i in warming_outputs]
                for loss in self.losses:
                    if loss in ["reid", "disappear"]:
                        continue
                    l_dict = self.get_loss(loss, aux_outputs, targets, warming_indices, w_num_masks)
                    l_dict = {"warming_" + k + f"_{i}": v for k, v in l_dict.items()}
                    losses.update(l_dict)
        else:
            len_aux = 0
        ...
        for loss in self.losses:
            if loss in ["reid", "disappear"]:
                continue
            l_dict = self.get_loss(loss, warming_outputs, targets, warming_indices, w_num_masks)
            l_dict = {"warming_" + k + f"_{len_aux}": v for k, v in l_dict.items()}
            losses.update(l_dict)
