...
class VideoInstanceCutter(nn.Module):
    ...
    def __init__(
        ...
        self.transformer_self_attention_layers_denoise = nn.ModuleList()
        self.transformer_cross_attention_layers_denoise = nn.ModuleList()
        self.transformer_ffn_layers_denoise = nn.ModuleList()
        ...
        for _ in range(3):
            self.transformer_self_attention_layers_denoise.append(
                SelfAttentionLayer(
                    d_model=hidden_dim,
                    nhead=num_head,
                    dropout=0.0,
            ...
            self.transformer_cross_attention_layers_denoise.append(
            ...
            self.transformer_ffn_layers_denoise.append(
                FFNLayer(
                    d_model=hidden_dim,
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher,
        ...
        frame_reid_embeds = frame_reid_embeds.permute(2, 3, 0, 1)
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                reference = self.reid_embed(single_frame_embeds_no_norm)
                all_reference.append(reference)

                frames_match_indices, noised_init = self.noiser(single_frame_embeds_no_norm, single_frame_embeds_no_norm, activate=False)
                output_1 = noised_init
                output_2 = single_frame_embeds_no_norm
                for j in range(3):
                    output_1 = self.transformer_cross_attention_layers_denoise[j](
                        indentify=output_1, tgt=reference,
                        key=single_frame_embeds_no_norm,
                        memory=single_frame_embeds_no_norm,
                    )
                    output_1 = self.transformer_self_attention_layers_denoise[j](output_1)
                    output_1 = self.transformer_ffn_layers_denoise[j](output_1)

                    output_2 = self.transformer_cross_attention_layers[j](
                        output_2, single_frame_embeds_no_norm
                    )
                    output_2 = self.transformer_self_attention_layers[j](output_2)
                    output_2 = self.transformer_ffn_layers_denoise[j](output_2)

                    warming_ms_output.append(output_1)
                    warming_ms_output.append(output_2)
                ...
                fused_output = (output_1 * activation1 + output_2 * activation2) / activation
                if self.training and random.random() < 0.5:
                    if random.random() < 0.5:
                        fused_output = fused_output * 0.0 + output_1
                    else:
                        fused_output = fused_output * 0.0 + output_2

                ms_output.append(fused_output)
                ...
                attn_mask = torch.zeros(size=(B, self.num_heads, pilot_pos.shape[0], fQ+self.track_queries.shape[0]),
                ...
                reference = self.reid_embed(self.track_queries)
                all_reference.append(reference)

                frames_match_indices, noised_init = self.noiser(self.track_queries, single_frame_embeds_no_norm, activate=True)  # TODO matching by reid_embeds
                output_1 = noised_init
                output_2 = self.track_queries
                for j in range(3):
                    output_1 = self.transformer_cross_attention_layers_denoise[j](
                        indentify=output_1, tgt=reference,
                        key=torch.cat([single_frame_embeds_no_norm, disappear_embeds], dim=0),
                        memory=torch.cat([single_frame_embeds_no_norm, disappear_embeds], dim=0),
                        query_pos=self.track_embeds, pos=torch.cat([detQ_pos, reference], dim=0),
                        memory_mask=attn_mask[:, :self.track_queries.shape[0], :]
                    )
                    output_1 = self.transformer_self_attention_layers_denoise[j](output_1)
                    output_1 = self.transformer_ffn_layers_denoise[j](output_1)

                    output_2 = self.transformer_cross_attention_layers[j](
                        output_2, torch.cat([single_frame_embeds_no_norm, disappear_embeds], dim=0),
                        query_pos=self.track_embeds, pos=torch.cat([detQ_pos, self.track_queries], dim=0),
                        memory_mask=attn_mask[:, :self.track_queries.shape[0], :],
                    )
                    output_2 = self.transformer_self_attention_layers[j](output_2)
                    output_2 = self.transformer_ffn_layers[j](output_2)
                ...
                pilot = torch.cat([fused_output, new_ins_embeds], dim=0)
                ...
                for j in range(3, self.num_layers):
                    output = self.transformer_cross_attention_layers[j](
                        ...
                        # pos=torch.cat([detQ_pos, fused_output], dim=0),
            ...
            warming_ms_output = torch.stack(warming_ms_output, dim=0)  # (3, q, b, c)
            warming_outputs_class, warming_outputs_mask = self.prediction(warming_ms_output, mask_features[:, i, ...])
            outputs_class, outputs_mask = self.prediction(ms_output, mask_features[:, i, ...])
            warming_out_dict = {
                "pred_logits": warming_outputs_class[-1],  # b, q', k+1
                "pred_masks": warming_outputs_mask[-1],  # b, q', h, w
                "aux_outputs": self._set_aux_loss(warming_outputs_class, warming_outputs_mask),
            ...
            if i == 0 and resume is False:
                ...
                warming_out_dict.update({
                    "indices": frames_info["indices"][i]
                })

                tgt2ref_indices = {tgt.item(): src.item() for src, tgt in zip(frames_info["indices"][i][0][0], frames_info["indices"][i][0][1])}
                all_tgt2ref.append(tgt2ref_indices)
                ...
                tgt2ref_indices = {tgt.item(): src.item() for src, tgt in zip(self.prev_frame_indices[0], self.prev_frame_indices[1])}
                all_tgt2ref.append(tgt2ref_indices)
            ...
            if self.num_reid_head_layers <= 0:
                ...
                out_dict.update({
                    ...
                    "disappear_tgt_id": -10000 if self.disappear_tgt_id is None else self.disappear_tgt_id,
                    "warming_outputs": warming_out_dict,
                })
            ...
            if i > 0:
                prev_ref = all_reference[-2]
                curr_ref = all_reference[-1]
                prev_tgt2ref = all_tgt2ref[-2]
                curr_tgt2ref = all_tgt2ref[-1]
                for tgt_id in prev_tgt2ref.keys():
                    if not tgt_id in curr_tgt2ref:
                        continue
                    anchor_embedding = curr_ref[curr_tgt2ref[tgt_id], 0, :]
                    positive_embedding = prev_ref[prev_tgt2ref[tgt_id], 0, :]
                    negative_query_id = sorted(
                        random.sample(set(range(curr_ref.shape[0])) - set([curr_tgt2ref[tgt_id]]),
                                      curr_ref.shape[0] - 1))
                    negative_embedding = curr_ref[negative_query_id, 0, :]
    ...
    def inference(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, frames_info, start_frame_id, resume=False, to_store="cpu"):
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                reference = self.reid_embed(self.track_queries)
                frames_match_indices, noised_init = self.noiser(self.track_queries, single_frame_embeds_no_norm, activate=False)
                output_1 = noised_init
                output_2 = self.track_queries
                for j in range(3):
                    output_1 = self.transformer_cross_attention_layers_denoise[j](
