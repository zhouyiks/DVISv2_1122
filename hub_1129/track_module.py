...
class VideoInstanceCutter(nn.Module):
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                ...
                detQ_pos, curr_mk, curr_a_sq = self.get_mask_pos_embed(
                    ...
                    mk=None, a_sq=None, mask_out=False)
            ...
            if not using_thr or self.num_reid_head_layers <= 0:
                ...
                self.track_embeds, _, _ = self.get_mask_pos_embed(
                    self.track_queries, outputs_mask[-1, ...], ori_mask_features[:, i, ...],
                    mk=curr_mk, a_sq=curr_a_sq, mask_out=False)
            ...
            if i > 0:
                tq_reid_embeds = self.reid_embed(self.track_embeds)  # q', b, c
    ...
    def get_mask_pos_embed(self, track_queries, mask_logits, mask_features, mk=None, a_sq=None, mask_out=False):
        ...
        qk = self.query_k(track_queries)
        qk = qk.permute(1, 2, 0)  # b, c, q
        if mk is None or a_sq is None:
            mk = self.mask_feature_k(mask_features)  # b, c, h, w
        ...
        for i in range(num_chunk):
            ...
            if mask_out:
                seg_mask = (mask_logits[:, start:end, :, :].sigmoid() > 0.5).to("cuda")  # b, q, n
                seg_mask = seg_mask.transpose(1, 2)  # b, n, q
