...
class VideoInstanceCutter(nn.Module):
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher,
        ...
        assert B == 1
        all_outputs = []
        continuously_disappearing = random.random() > 0.7
            ...
            if i == 0 and resume is False:
                ...
                if not using_thr:
                    ...
                    if self.disappear_tgt_id is not None:
                        if continuously_disappearing and random.random() < 0.7:
                            aux_tgt_i_for_each_fq = frames_info["aux_indices"][i][0][1]
                            disappear_fq_mask[aux_tgt_i_for_each_fq == self.disappear_tgt_id] = True
                            tgt2src_dict = {tgt: src for tgt, src in zip(self.prev_frame_indices[1],
                                                                         self.prev_frame_indices[0])}
                            self.disappear_trcQ_id = tgt2src_dict[self.disappear_tgt_id]
                        else:
                            continuously_disappearing = False
                            self.disappear_tgt_id = None
                            self.disappear_trcQ_id = None
                    elif len(self.disappeared_tgt_ids) > 0:
                        self.disappear_tgt_id = None
                        self.disappear_trcQ_id = None
