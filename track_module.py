...
class VideoInstanceCutter(nn.Module):
    ...
    def __init__(
        ...
        # self.transformer_self_attention_layers_propagate = nn.ModuleList()
        # self.transformer_cross_attention_layers_propagate = nn.ModuleList()
        # self.transformer_ffn_layers_propagate = nn.ModuleList()

        self.transformer_self_attention_layers = nn.ModuleList()
        self.transformer_cross_attention_layers = nn.ModuleList()
        self.transformer_ffn_layers = nn.ModuleList()

        for _ in range(warming_layer_num):
        ...
        for _ in range(warming_layer_num + 3):
    ...
    def frame_forward(self, track_embeds, single_frame_embeds, disappear_embeds=None,
        ...
        noised_init = self.noiser(
            track_embeds,
            single_frame_embeds,
            activate=activate,
            frame_info=None  # {"valid_mask": valid_fq_mask}
        )
        output_1 = noised_init
        output_2 = track_embeds

        attn_mask_1 = torch.zeros(size=(B, self.num_heads, track_embeds.shape[0], fQ + track_embeds.shape[0]), dtype=torch.bool).to("cuda")
        attn_mask_1[..., :fQ] = ~valid_fq_mask[None, None, None, :].repeat(B, self.num_heads, track_embeds.shape[0], 1)
        attn_mask_1 = attn_mask_1.flatten(0, 1)
        warming_ms_output = []
        for i in range(self.num_warming_layers):
            output_1 = self.transformer_cross_attention_layers_denoise[i](
                indentify=output_1, tgt=track_embeds,
                key=single_frame_embeds if disappear_embeds is None else torch.cat([single_frame_embeds, disappear_embeds], dim=0),
                memory=single_frame_embeds if disappear_embeds is None else torch.cat([single_frame_embeds, disappear_embeds], dim=0),
                query_pos=trcQ_pos, pos=detQ_pos if disappear_embeds is None else torch.cat([detQ_pos, track_embeds], dim=0),
                memory_mask=attn_mask_1[..., :fQ] if disappear_embeds is None else attn_mask_1
            )
            output_1 = self.transformer_self_attention_layers_denoise[i](output_1)
            output_1 = self.transformer_ffn_layers_denoise[i](output_1)

            output_2 = self.transformer_cross_attention_layers[i](
                tgt=output_2,
                memory=single_frame_embeds if disappear_embeds is None else torch.cat([single_frame_embeds, disappear_embeds], dim=0),
                query_pos=trcQ_pos, pos=detQ_pos if disappear_embeds is None else torch.cat([detQ_pos, track_embeds], dim=0),
                memory_mask=attn_mask_1[..., :fQ] if disappear_embeds is None else attn_mask_1
            )
            output_2 = self.transformer_self_attention_layers[i](output_2)
            output_2 = self.transformer_ffn_layers[i](output_2)
        ...
        pilot = fused_output if disappear_embeds is None else torch.cat([fused_output, new_ins_embeds], dim=0)
        pilot_pos = trcQ_pos if disappear_embeds is None else torch.cat([trcQ_pos, detQ_pos], dim=0)
        attn_mask_2 = torch.zeros(size=(B, self.num_heads, pilot.shape[0], fQ+fused_output.shape[0]), dtype=torch.bool).to("cuda")
        attn_mask_2[:, :, :fused_output.shape[0], :fQ] = ~valid_fq_mask[None, None, None, :].repeat(B, self.num_heads, fused_output.shape[0], 1)
        attn_mask_2[:, :, fused_output.shape[0]:, fQ:] = True
        attn_mask_2 = attn_mask_2.flatten(0, 1)
        ms_output = []
        ms_output.append(pilot)
        for i in range(self.num_warming_layers, self.num_warming_layers+3):
            output = self.transformer_cross_attention_layers[i](
                tgt=ms_output[-1],
                memory=single_frame_embeds if disappear_embeds is None else torch.cat([single_frame_embeds, disappear_embeds], dim=0),
                query_pos=pilot_pos, pos=detQ_pos if disappear_embeds is None else torch.cat([detQ_pos, track_embeds], dim=0),
                memory_mask=attn_mask_2[..., :fQ] if disappear_embeds is None else attn_mask_2
            )
            output = self.transformer_self_attention_layers[i](output)
            output = self.transformer_ffn_layers[i](output)
            ms_output.append(output)

        warming_ms_output = torch.stack(warming_ms_output, dim=0)
        ms_output = torch.stack(ms_output, dim=0)
        return warming_ms_output, ms_output
    ...
    def forward(self, frame_embeds_no_norm, mask_features, targets, frames_info, matcher, resume=False, using_thr=False):
        ...
        for i in range(T):
            ...
            if i == 0 and resume is False:
                self._clear_memory()
                warming_ms_output, ms_output = self.frame_forward(
