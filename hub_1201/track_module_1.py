...
class VideoInstanceCutter(nn.Module):
    ...
    def __init__(
        ...
        self.new_ins_embeds = nn.Embedding(self.num_new_ins, hidden_dim)
    ...
    def forward(self, frame_embeds_no_norm, frame_reid_embeds, mask_features, targets, frames_info, matcher,
        ...
        new_ins_embeds = self.new_ins_embeds.weight.unsqueeze(1).repeat(1, B, 1)  # nq, b, c
            ...
            if i == 0 and resume is False:
                ...
                for j in range(self.num_layers):
                    output = self.transformer_cross_attention_layers[j](
                        ...
                        # query_pos=pilot_pos,
                        pos=torch.cat([torch.zeros_like(single_frame_embeds_no_norm),
